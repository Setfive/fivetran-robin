...
export const handler = async (
  ...
  context: Context,
): Promise<IFivetranResult | IFivetranError> => {
  ...
  console.log(
    `Cursor: ${request.state.transactionsCursor}, Destination: s3://${request.bucket}/${request.file}`,
  );
...
async function doFetchRecords(
  ...
  try {
    ...
    if (axios.isAxiosError(e)) {
      ...
      return {
        success: false,
        error: `Robin API Error: HTTP ${axiosError.code} ${JSON.stringify(
    ...
    try {
      ...
      if (axios.isAxiosError(e)) {
        ...
        if (axiosError.code !== 'ERR_BAD_REQUEST') {
          return {
            success: false,
            error: `Robin API Error: HTTP (${axiosError.status}) ${
              axiosError.code
...
async function sleep(sleepTimeMs: number) {
  return new Promise<void>((resolve, reject) =>
    setTimeout(resolve, sleepTimeMs * 1000),
  );
}
...
async function fetchRecords(
  ...
  if (createdAtDates.length) {
    ...
    if (request.state.transactionsCursor) {
      const transactionsCursorMoment = moment(request.state.transactionsCursor);
      useLastCreatedAt = createdAtDates[createdAtDates.length - 1].isAfter(
        transactionsCursorMoment,
      );
    }
